...
static void      _print_image(const struct mach_header *mh, bool added) {
  ...
  NSString *loader = [NSString stringWithUTF8String:image_name];
  [loadedImages addObject:@{@"name":loader.lastPathComponent,
                            @"filetype":_describe_filetype(mh),
                            @"path": loader,
                            @"ncmds":@(mh->ncmds)}.mutableCopy];
...
static NSString * _describe_filetype(const struct mach_header *mh){
...
static void    _image_visit_load_commands(const struct mach_header *mh,
                  void (^visitor)(struct load_command *lc, bool *stop)) {
	assert(visitor != NULL);
